...
def rating_to_label(rating):
    """Map Foody rating to sentiment label.

    Note: this follows the user's current rule. It only creates
    negative/positive labels from rating. Neutral samples are used only
    when rating_label is already available in the JSONL record.
    """
    if rating >= 8.0:
        return "positive"
    return "negative"
